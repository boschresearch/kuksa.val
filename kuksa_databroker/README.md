...
- [Kuksa Databroker](#kuksa-databroker)
  ...
  - [Running KUKSA.val Databroker](#running-kuksaval-databroker)
    - [:warning: Default port not working on Mac OS](#warning-default-port-not-working-on-mac-os)
  - [Test the Databroker using CLI](#test-the-databroker-using-cli)
    - [Authorization](#authorization)
    - [Connect to databroker using TLS](#connect-to-databroker-using-tls)
    - [Data Broker Query Syntax](#data-broker-query-syntax)
  ...
  - [GRPC Interfaces](#grpc-interfaces)
    - [`kuksa.val.v1.VAL`](#kuksavalv1val)
    - [`sdv.databroker.v1.Broker`](#sdvdatabrokerv1broker)
    - [`sdv.databroker.v1.Collector`](#sdvdatabrokerv1collector)
  - [Signal Change Types](#signal-change-types)
...
This GRPC interface is under development and is  used by kuksa-databroker, kuksa-client library and the providers.

### `sdv.databroker.v1.Broker`

This interface is still supported but not currently under active development. The RUAST databroker-cli supports this interface. It is defined as follows (see [file](proto/sdv/databroker/v1/broker.proto) in the proto folder):
...
There is also a [Collector](proto/sdv/databroker/v1/collector.proto) interface which is used by legacy providers to feed data into the broker.
...
Internally, databroker knows different change-types of VSS signals. There are thee change-types

 - **Continuos**: This are usefully sensor values that are - well - continuos, such as vehicle speed. Whenever a continuos signal is updated by a provider, all subscribers are notified.
